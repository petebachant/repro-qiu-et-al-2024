...
idir = "/pool001/lyqiu/Siting_Optimization/NE_2024/Meteorological_Data/Low/Solar/netcdf"
odir = "/pool001/lyqiu/Siting_Optimization/NE_2024/Meteorological_Data/Low/Solar"
sy=2007
ey=2013
lct=-5
# from ERA5 geopotentail
altitude = xr.open_dataset(f'{idir}/ERA5_altitude.nc')['z'].squeeze()
mask = xr.open_dataset(f'/pool001/lyqiu/Siting_Optimization/NE_2024/Meteorological_Data/Low/ERA5_mask_ge50.nc')['mask'].squeeze()  # from ERA5 lsm, taking >=0.85 as land
...
for iy in range(sy,ey+1):
    ...
    ds_new["Wind Speed"] = np.sqrt(ds["u10"] ** 2 + ds["v10"] ** 2).assign_attrs(
    ...
    for i in range(lats.size):
        for j in range(lons.size):
            if mask[i,j]==1:
                ...
                dd = dd.tz_localize("UTC", level=0).tz_convert("US/Eastern") # convert to local time
                ...
                lonstr = '%.2f' % (lon)
                latstr = '%.2f' % (lat)
                csvname = f"{odir}/{iy}/{lid}_{latstr}_{lonstr}_{iy}.csv"
