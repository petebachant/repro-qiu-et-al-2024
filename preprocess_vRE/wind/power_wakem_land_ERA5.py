...
cdo = Cdo()
mdl="ERA5"
idir = f"/nobackup1/lyqiu/WindUncertainty/Data/ERA5/original/"
...
for ca in cases:
    ...
    datafile = f"{idir}/{wsh}m/{mdl}_uv{wsh}_ISNE_{iy}_masked.nc"
    for degree in [0.06]:
        ...
        uv_interpolated = cdo.remapnn(maskfilen, input="-mulc,%f -select,month=%d %s" % (ex, im, datafile), returnXDataset=True)
        ws, wd = wswd_from_uv(uv_interpolated['u'], uv_interpolated['v'])
        ...
        POWERS_3d = ws
        maskd=read_netcdf(maskfilen,'mask')
    ...
    POWERS_3d.to_netcdf("%s/%s_wakecf%d_%s_%s_%.2fdeg_%d_%d.nc"%(outputdir,mdl,Nt,wtmdl,ca,degree,iy,im))
