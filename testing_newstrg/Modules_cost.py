...
def add_constraints(dat, Setting, Model):
    ...
    for dpland in Setting.gas_plant_types:
        dindex = Setting.gas_plant_types.index(dpland)
        pindex = Setting.plant_types.index(dpland)
        Model.addConstrs(DV.prod[pindex, t] <= DV.Xg[dindex]
                         for t in range(nT))
    
    # c2: upper bound for renewable energy generation capacity
    for repland in Setting.RE_plant_types:
        rindex = Setting.RE_plant_types.index(repland)
        pindex = Setting.plant_types.index(repland)
        for t in range(nT):
    ...
    for dpland in Setting.gas_plant_types:
        ...
        Model.addConstr(ff_load <= dat.demand.sum(
        )*Setting.UB_dispatchable_cap[dpland], name=f'c_ffload_cap_{dpland}')
    ...
    #no lost load
    Model.addConstrs(DV.LL[t] == 0 for t in range(nT))
    ...
    Model.addConstrs(DV.sLev[t] <= DV.Xstr for t in range(0, nT))
